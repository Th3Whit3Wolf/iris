import { SpectrumAnalyzerController } from "#components";
...
describe("AnalyzerControl", () => {
	...
	it("should have functioning buttons", () => {
		...
		// NOTE: ‹ is not < symbol!
		const buttonGhz = SpecA.queryAllByRole("button").filter(
			b => b.textContent === "‹"
		)[0];
		const buttonMhz = SpecA.queryAllByRole("button").filter(
			b => b.textContent === "‹"
		)[1];
		const buttonKhz = SpecA.queryAllByRole("button").filter(
			b => b.textContent === "‹"
		)[2];
		...
		button3.click();
		const button0 = SpecA.queryAllByRole("button").find(
			b => b.textContent === "0"
		);
		button0.click();
