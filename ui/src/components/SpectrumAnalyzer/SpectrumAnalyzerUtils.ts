...
const handleDrawInstructorSignals = (
	self: SpectrumAnalyzerInterface,
	color: Color,
	signal: Signal
) => {
	// Calculate 2 Signals
...
const handleDrawStudentSignals = (
	...
	if (!self.isRfMode) {
		...
		ifDownSignal.freq += self.loopback
			? +self.antennaOffset
			: self.targetOffset;
		ifDownSignal.amp = !self.loopback && !self.hpa ? -1000 : ifDownSignal.amp;
		...
		rfDownSignal.freq += self.loopback
			? +self.antennaOffset
			: self.targetOffset;
		rfDownSignal.amp = !self.loopback && !self.hpa ? -1000 : rfDownSignal.amp;
