...
import { SpectrumAnalyzer } from '#components';
import { useLayoutEffect, useState , useEffect } from 'react';
// import { useTheme } from '@mui/material/styles/useTheme';
import { satellites } from '#constants';
import { useAntennaContext, useAppContext } from '#context';
...
const configButtonStyle = {
  ...
  cursor: 'pointer',
  '&:hover': {
    // backgroundColor: theme.palette.serious.main,
  },
};
...
const SpectrumAnalyzerBox = props => {
  ...
  const {app, setApp: updateAppState} = useAppContext();
  const whichSpecA = props.canvasId.split('A')[1];

  useEffect(() => {
    window.sewApp.socket.on('updateSpecA', (data: any) => {
  ...
  useLayoutEffect(() => {
    const canvasDom = document.getElementById(props.canvasId) as HTMLCanvasElement;
    canvasDom.width = canvasDom.parentElement?.offsetWidth ? canvasDom?.parentElement.offsetWidth - 6 : 0;
    canvasDom.height = canvasDom.parentElement?.offsetWidth ? canvasDom?.parentElement.offsetWidth - 6 : 0;
