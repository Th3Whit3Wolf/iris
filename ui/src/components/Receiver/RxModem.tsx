...
import { useAntennaContext, useTxContext, useRxContext, useSignalContext, useUserContext } from '#context';
...
const RxModem = ({ unit }) => {
  ...
  const unitData = rx.filter(x => x.unit == unit && x.team_id == user.team_id && x.server_id == user.server_id)
  const {signal: signalData} = useSignalContext();
  ...
  const RxModemInput = () => {
    ...
    const currentRow = rx.map(x => x.id).indexOf(unitData[currentModem].id);
  ...
  const RxVideo = () => {
    ...
    useEffect(() => {
      ...
      const { frequency: r_freq, bandwidth: r_bw, modulation: r_mod, fec: r_fec } = rx[currentRow]; 
      //console.log('rx: ', rxContext[currentRow])
      const { target_id: r_tgt, band: r_band } = antenna[rx[currentRow].antenna_id - 1];
