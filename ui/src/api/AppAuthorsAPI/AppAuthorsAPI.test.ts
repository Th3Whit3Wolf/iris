...
describe.concurrent("ServerAPI", () => {
    ...
    const emptyName = "";
    const longName = "This is a very long string that is longer than 250 characters. Why would you enter a name so long? I see no need for a name that is so long, perhaps your parents wanted to win some kind of record? You will need to truncate your name so that is smaller than 250 characters.";
    ...
    test(".first_name() should fail when given an empty string", () => {
		expect(() => api.first_name(emptyName)).toThrowError("[AppAuthorsAPI::first_name] Error(Invalid Type):\nExpected: string with a length between 0 & 250.\nReceived: \n");
    });

    test(".first_name() should fail when given a string over 250 characters", () => {
		expect(() => api.first_name(longName)).toThrowError(`[AppAuthorsAPI::first_name] Error(Invalid Type):\nExpected: string with a length between 0 & 250.\nReceived: ${longName}\n`);
    });
    ...
    test(".last_name() should fail when given an empty string", () => {
		expect(() => api.last_name(emptyName)).toThrowError("[AppAuthorsAPI::last_name] Error(Invalid Type):\nExpected: string with a length between 0 & 250.\nReceived: \n");
    });

    test(".last_name() should fail when given a string over 250 characters", () => {
		expect(() => api.last_name(longName)).toThrowError(`[AppAuthorsAPI::last_name] Error(Invalid Type):\nExpected: string with a length between 0 & 250.\nReceived: ${longName}\n`);
    });
});